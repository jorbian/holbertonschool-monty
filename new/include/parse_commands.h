...
/**
 * struct CommandParser - Struct defining the way commands are parsed
 *
 * @raw_input: the last line of input as a string
 * @error_tripped: If the last function returned an error (1 for Yes or 0 No)
 * @tokens: parsed command tokens in doubly linked list
 * Each cell of this array is a pointer to the first node of a linked list,
 * because we want our HashTable to use a Chaining collision handling
 */
...
typedef void (*Command)(SSTAck_t **stack_pointer, unsigned int line_number);
...
/* METHODS (I.E. FUNCTIONS) FOR CREATING AND USING COMMANDPARSER */
